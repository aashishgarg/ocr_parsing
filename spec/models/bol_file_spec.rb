...
RSpec.describe BolFile, type: :model do
  ...
  context '#validations' do
    it 'does not allow blank [status]' do
      bol_file = FactoryBot.build(:bol_file, user: @user, status: nil)
      expect(bol_file.valid?).to eq(false)
    end

    it 'with [status]' do
      bol_file = FactoryBot.build(:bol_file, user: @user)
      expect(bol_file.valid?).to eq(true)
    end

    it 'with no name' do
      bol_file = FactoryBot.build(:bol_file, user: @user, name: nil)
      expect(bol_file.valid?).to eq(false)
    end
  ...
  context '#Callback' do
    context 'when status = qa_approved' do
      before do
        @bol_file = FactoryBot.create(:bol_file, user: @user, name: 'BolFile Name')
